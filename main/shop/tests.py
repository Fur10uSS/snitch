...
class TestDataBase(TestCase):
    ...
    # def test_cart_older_7_days(self):
        ...
        1. get cart and make if older
        ===============================================
        Add some code to @staticmethod Order.get_cart(user)
        """
        # cart = Order.get_cart(self.user)
        # cart.creation_time = timezone(2000, 1, 1, tzinfo=zoneinfo.ZoneInfo('UTC'))
        # cart.save()
        # cart = Order.get_cart(self.user)
    ...
    def test_recalculate_order_amount_after_changing_orderitem(self):
        ...
        # 1. Get order amount before any changing
        cart = Order.get_cart(self.user)
        self.assertEqual(cart.amount, decimal.Decimal(0))
        ...
        cart = Order.get_cart(self.user)
        self.assertEqual(cart.amount, decimal.Decimal(10))

        # 3. ----------""--------- after deleting item
        i.delete()
        cart = Order.get_cart(self.user)
        self.assertEqual(cart.amount, decimal.Decimal(4))
    ...
    def test_method_get_amount_of_unpaid_orders(self):
        ...
        self.assertEqual(amount, decimal.Decimal(0)) 
    ...
    def test_method_get_balance(self):
        ...
        self.assertEqual(amount, decimal.Decimal(9))

        # 2. After adding payment
        Payment.objects.create(user=self.user, amount=100)
        amount = Payment.get_balance(self.user)
        self.assertEqual(amount, decimal.Decimal(109))

        # 3. After adding same payments
        Payment.objects.create(user=self.user, amount=-50)
        amount = Payment.get_balance(self.user)
        self.assertEqual(amount, decimal.Decimal(59))
    ...
    def test_auto_payment_after_apply_make_order_true(self):
        ...
        cart = Order.get_cart(self.user)
        OrderItem.objects.create(order=cart, product=self.p, price=2, quantity=2)
        self.assertEqual(Payment.get_balance(self.user), decimal.Decimal(9))
        cart.make_order()
        self.assertEqual(Payment.get_balance(self.user), decimal.Decimal(5))
    ...
    def test_auto_payment_after_add_required_payment(self):
        ...
        self.assertEqual(Payment.get_balance(self.user), decimal.Decimal(565))
        amount = Order.get_amount_of_unpaid_orders(self.user)
        self.assertEqual(amount, decimal.Decimal(0))
    ...
    def test_auto_payment_for_earlier_order(self):
        ...
        self.assertEqual(Payment.get_balance(self.user), decimal.Decimal(1009))
        amount = Order.get_amount_of_unpaid_orders(self.user)
        self.assertEqual(amount, decimal.Decimal(0))
    ...
    def test_auto_payment_for_all_orders(self):
        ...
        self.assertEqual(Payment.get_balance(self.user), decimal.Decimal(10009))
        amount = Order.get_amount_of_unpaid_orders(self.user)
        self.assertEqual(amount, decimal.Decimal(0))
